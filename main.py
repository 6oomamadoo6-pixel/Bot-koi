...
def generate_anon_code():
    while True:
        ...
        conn = db()
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM users WHERE anon_code = ?", (code,))
        exists = cur.fetchone()
        conn.close()
        if not exists:
            return code
...
def generate_unblock_code():
    while True:
        ...
        conn = db()
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM blocks WHERE unblock_code = ?", (code,))
        exists = cur.fetchone()
        conn.close()
        if not exists:
            return code
...
def get_or_create_user(user_id, username, full_name):
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT link_code, anon_code FROM users WHERE user_id = ?", (user_id,))
    row = cur.fetchone()
...
def get_user(user_id):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT user_id, username, full_name, link_code, anon_code, display_name
...
def get_display_name(user_id):
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT display_name, full_name FROM users WHERE user_id = ?", (user_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return "کاربر"
...
def save_anonymous_message(sender_id, receiver_id, message_text):
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO anonymous_messages (sender_id, receiver_id, message_text, created_at)
        VALUES (?, ?, ?, ?)
    """, (sender_id, receiver_id, message_text, datetime.now().isoformat()))
...
def is_blocked(blocker_id, blocked_id):
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM blocks WHERE blocker_id = ? AND blocked_id = ?", (blocker_id, blocked_id))
    result = cur.fetchone()
    conn.close()
...
def unblock_by_code(user_id, code):
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT blocked_id FROM blocks WHERE blocker_id = ? AND unblock_code = ?", (user_id, code))
    row = cur.fetchone()
    if not row:
        conn.close()
        return False
    cur.execute("DELETE FROM blocks WHERE blocker_id = ? AND unblock_code = ?", (user_id, code))
    conn.commit()
    conn.close()
    return True
...
def get_block_list(user_id):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT users.anon_code, blocks.unblock_code
        FROM blocks
        JOIN users ON users.user_id = blocks.blocked_id
...
async def is_member(bot, user_id):
    first = await check_channel_member(bot, CHANNEL_1, user_id)
    second = await check_channel_member(bot, CHANNEL_2, user_id)
...
def join_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("چنل 2 : کانال ( اجباری ) 📢", url=CHANNEL_2_URL)],
...
def main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("تنظیمات نام ⚙️", callback_data="name_settings"),
            InlineKeyboardButton("تبلیغات 📢", callback_data="ads")
        ],
        [InlineKeyboardButton("لیست مسدودی 🔴", callback_data="block_list")],
        [
            InlineKeyboardButton("چنل پشتیبانی ✅", url=CHANNEL_1_URL),
...
def name_settings_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("تغییر نام ✏️", callback_data="change_name")],
...
def anonymous_message_keyboard(message_id, sender_id):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("پاسخ 🤠", callback_data=f"reply:{message_id}"),
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_join":
        if await is_member(context.bot, user_id):
            ...
            await query.answer("هنوز در هر دو کانال جوین نشده‌ای ❌", show_alert=True)
        return
    ...
    if data == "cancel_send":
        context.user_data.clear()
        await query.edit_message_text("❌ ارسال پیام ناشناس لغو شد.", reply_markup=back_keyboard())
        return
    ...
    if data == "name_settings":
        await name_settings(update, context)
        return
    ...
    if data == "ads":
        await ads_page(update, context)
        return

    if data == "block_list":
        await block_list_page(update, context)
        return
    ...
    if data.startswith("reply:"):
        try:
            anonymous_message_id = int(data.split(":", 1)[1])
        except (ValueError, IndexError):
            await query.answer("پیام نامعتبر است ❌", show_alert=True)
            return

        anonymous_message = get_anonymous_message(anonymous_message_id)
        if not anonymous_message:
            await query.answer("این پیام دیگر موجود نیست ❌", show_alert=True)
            return
        ...
        if receiver_id != user_id:
            await query.answer("این پیام متعلق به شما نیست ❌", show_alert=True)
            return

        if is_blocked(user_id, sender_id):
            await query.answer("این کاربر را بلاک کرده‌ای.", show_alert=True)
            return
        ...
        await query.message.reply_text("پاسخ خود را بنویسید: ✍️", reply_markup=cancel_keyboard())
        return

    if data.startswith("block:"):
        try:
            blocked_id = int(data.split(":", 1)[1])
        except (ValueError, IndexError):
            await query.answer("کاربر نامعتبر است ❌", show_alert=True)
            return

        if blocked_id == user_id:
            await query.answer("نمی‌توانی خودت را بلاک کنی 😅", show_alert=True)
            return

        success = block_user(user_id, blocked_id)
        if success:
            await query.edit_message_text("🔴 کاربر با موفقیت بلاک شد.", reply_markup=back_keyboard())
        else:
            await query.answer("این کاربر قبلاً بلاک شده است.", show_alert=True)
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("unblock_") and text.endswith("/"):
        ...
        if not code:
            await message.reply_text("❌ کد رفع مسدودی نامعتبر است.")
            return
        success = unblock_by_code(user.id, code)
        if success:
            await message.reply_text("🟢 کاربر با موفقیت از لیست مسدودی شما خارج شد.")
        else:
    ...
    if context.user_data.get("waiting_broadcast") and user.id == ADMIN_ID:
        ...
        status_msg = await message.reply_text(
            f"⏳ در حال ارسال پیام همگانی به {len(users)} کاربر...\nلطفا صبر کن."
        )
    ...
    if context.user_data.get("replying"):
        ...
        if not anonymous_message_id or not sender_id:
            context.user_data.clear()
            await message.reply_text("❌ اطلاعات پیام پیدا نشد.", reply_markup=back_keyboard())
            return

        anonymous_message = get_anonymous_message(anonymous_message_id)
        if not anonymous_message:
            context.user_data.clear()
            await message.reply_text("❌ پیام اصلی پیدا نشد.", reply_markup=back_keyboard())
            return
        ...
        if receiver_id != user.id or original_sender_id != sender_id:
            context.user_data.clear()
            await message.reply_text("❌ خطا در اطلاعات پیام.")
            return

        if is_blocked(user.id, sender_id):
            context.user_data.clear()
            await message.reply_text("❌ این کاربر را بلاک کرده‌اید.")
            return

        sender_name = get_display_name(user.id)
        reply_text = (
            f"کاربر {sender_name} به پیام شما پاسخ داد. 🤠\n\n"
            f"پیام شما :\n{original_text}\n\n"
            f"پیام پاسخ داده شده :\n{text}"
        )

        try:
            await context.bot.send_message(chat_id=sender_id, text=reply_text)
    ...
    if context.user_data.get("sending_anonymous"):
        ...
        if not target_id:
            context.user_data.clear()
            await message.reply_text("❌ مقصد پیام پیدا نشد.", reply_markup=back_keyboard())
            return

        if target_id == user.id:
            context.user_data.clear()
            await message.reply_text("❌ نمی‌توانی به خودت پیام بفرستی.", reply_markup=back_keyboard())
            return
        ...
        keyboard = anonymous_message_keyboard(anonymous_message_id, user.id)
